...
class FTGAN_Layer_Atten(nn.Module):
    def __init__(self, ngf=64):
        ...
        self.ngf = ngf
        self.fc = nn.Linear(2048, 3) #nn.Linear(4096, 3)
        self.softmax  = nn.Softmax(dim=1) 
    ...
    def forward(self, style_features, style_features_1, style_features_2, style_features_3, B, K):
        ...
        #print('>>>', style_features.shape, self.fc)
...
class FTGAN_Generator_MLAN(nn.Module):
    ...
    def forward(self, inp, ensemble=False):
        content_image, style_images = inp
        B, K, _, _ = style_images.shape
        npass = 1
        if ensemble:
        ...
        out = []
        noise = 0.1
        content_feature_zeros = torch.empty([1,128,16,16])
        style_features_zeros = torch.empty([68,128,16,16])
        style_features_1_zeros = torch.empty([68,128]) #.normal_(0, noise).cuda()
        style_features_2_zeros = torch.empty([68,128])
        style_features_3_zeros = torch.empty([68,128])
        for _ in range(npass):
            content_feature = self.content_encoder(content_image)
            if ensemble:
                content_feature += content_feature_zeros.normal_(0, noise).cuda()

            style_features = self.style_encoder(style_images.view(-1, 1, 64, 64))
            if ensemble:
                style_features += style_features_zeros.normal_(0, noise).cuda()
            style_features_1 = self.local_atten_1(style_features)
            if ensemble:
                style_features_1 += noise*torch.randn([68,128]).cuda() #style_features_1_zeros.normal_(0, noise).cuda()

            style_features = self.downsample_1(style_features)
            style_features_2 = self.local_atten_2(style_features)
            if ensemble:
                style_features_2 += noise*torch.randn([68,128]).cuda() #style_features_2_zeros.normal_(0, noise).cuda()
        
            style_features = self.downsample_2(style_features)
            style_features_3 = self.local_atten_3(style_features)
            if ensemble:
                style_features_3 += noise*torch.randn([68,128]).cuda() #style_features_3_zeros.normal_(0, noise).cuda()

            style_features = self.layer_atten(style_features, style_features_1, style_features_2, style_features_3, B, K)
            feature = torch.cat([content_feature, style_features], dim=1)
            if ensemble:
                feature += noise*torch.randn([1, 256, 16, 16]).cuda() #torch.empty(feature.shape).normal_(0, noise).cuda()
            
            out.append(self.decoder(feature))
        
        if ensemble:
            std = torch.std(torch.stack(out),0)
            mean = torch.mean(torch.stack(out),0)
            outp = mean #torch.div(mean,std)
        else:
            #outp = self.decoder(feature)
            outp = self.decoder(out[0])
        return outp    
